...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  // =================================================
  // syntax to iterate every word in words vector
  for(const std::string& word : words){
    // in the wordcount_map this word is not find namely is its first time to appear
    if(!wordcount_map.count(word)){
      wordcount_map[word] = 1;
    }
    else{
      // has it in the map already, add the value by one 
     wordcount_map[word] += 1;
    }
  }
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  // =================================================
  //the at function will search for a given key and return a
